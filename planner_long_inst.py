    ...
    def _generate_plan_(self):
        if self.task['action'] == 'push_both':
            return [
                ...
                Move(self.env, [0, 100]),
                Release(self.env),
                Reach(self.env, self.task['target'][1]),
                Grip(self.env),
                Move(self.env, [0, 100]),]
            ...
            return [
                ...
                Reach(self.env, self.task['target'][0]),
                Grip(self.env),
                Reach(self.env, self.task['target'][2]),
                Release(self.env),
                Reach(self.env, self.task['target'][1]),
                Grip(self.env),
                Reach(self.env, self.task['target'][2]),]
            ...
            return [
                ...
                Reach(self.env, self.task['target'][0]),
                Grip(self.env),
                MoveABS(self.env, np.array([-100, 200])),
                Release(self.env),
                Reach(self.env, self.task['target'][1]),
                Grip(self.env),
                MoveABS(self.env, np.array([0, 200])),
                Release(self.env),
                Reach(self.env, self.task['target'][2]),
                Grip(self.env),
                MoveABS(self.env, np.array([100, 200])),]
            ...
            actions = []
            for i in range(len(self.task['target'])):
                actions.append(Release(self.env))
                actions.append(Reach(self.env, self.task['target'][i]))
                actions.append(Grip(self.env))
                actions.append(Move(self.env, [0, 100]))
            return actions
